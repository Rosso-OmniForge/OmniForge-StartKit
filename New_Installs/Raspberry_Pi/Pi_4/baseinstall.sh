...
detect_os() {
    ...
    if [ -f /proc/device-tree/model ]; then
        ...
        if [[ $MODEL == *"Raspberry Pi 5"* ]]; then
            IS_PI4=true
            log_info "Running on Raspberry Pi 4 - will apply specific optimizations"
...
optimize_pi5_nvme() {
    if [ "$IS_PI4" != true ] || [ "$STORAGE_TYPE" != "nvme" ]; then
        return
    fi
...
post_install_summary() {
    ...
    if [ "$IS_PI4" = true ] && command -v vcgencmd &> /dev/null; then
