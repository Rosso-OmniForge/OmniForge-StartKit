...
apply_pi5_fixes() {
    ...
    log_info "Applying Raspberry Pi 5 NVMe stability fixes..."
    
    # Backup config.txt if not already backed up
    if [ ! -f /boot/firmware/config.txt.backup ]; then
        log_info "Backing up /boot/firmware/config.txt..."
        cp /boot/firmware/config.txt /boot/firmware/config.txt.backup
        log_success "Backup created: /boot/firmware/config.txt.backup"
    fi
...
install_redteam_tools() {
    log_info "Installing Red Team / Penetration Testing tools..."
...
post_install_summary() {
    echo ""
    echo "================================================================================"
    log_success "BASE INSTALLATION COMPLETED SUCCESSFULLY"
    echo "================================================================================"
    echo ""
    ...
    if [ "$IS_PI5" = true ]; then
        ...
        echo "  ⚠️  A REBOOT IS REQUIRED for NVMe fixes to take effect"
        echo ""
    fi
    ...
    echo "     gh auth login"
    echo ""
    echo "  2. Mount Windows shares now (or reboot):"
    echo "     sudo mount -a"
    echo ""
    echo "  3. Web Server Quick Start:"
    ...
    echo ""
    echo "  3. Verify NVMe health (Pi5 only):"
    echo "     sudo smartctl -a /dev/nvme0n1"
    echo ""
    echo "  4. Test Red Team tools as needed"
    echo ""
    
    if [ "$IS_PI5" = true ]; then
        log_warning "REBOOT NOW to activate Pi5 NVMe optimizations:"
        echo "     sudo reboot"
        echo ""
    fi
...
main() {
    clear
    echo "================================================================================"
    echo "  BASE INSTALLATION SCRIPT - DEBIAN/UBUNTU/RASPBERRY PI OS"
    echo "  Red Team Engineer Setup"
    echo "================================================================================"
    echo ""
    ...
    # Install web cloning and social engineering tools
    install_web_cloning_tools
